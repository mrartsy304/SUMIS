...
import { departmentAPI, eventsAPI, announcementsAPI } from "../services/api";
...
export default function AdminPortal() {
  ...
  // Events & Announcements (for overview stats)
  ...
  useEffect(() => {
    fetchUsers();
    fetchDepartments();
    eventsAPI.getAll().then(r => setEvents(Array.isArray(r.data) ? r.data : [])).catch(() => {});
    announcementsAPI.getAll().then(r => setAnnouncements(Array.isArray(r.data) ? r.data : [])).catch(() => {});
  ...
  const addStep = () => setForm(f => ({
    ...f, steps: [...f.steps, { step_number: f.steps.length + 1, title: "", description: "", note: "" }],
  }));

  const removeStep = (idx) => setForm(f => ({
    ...f, steps: f.steps.filter((_, i) => i !== idx).map((s, i) => ({ ...s, step_number: i + 1 })),
  }));

  const updateStep = (idx, field, value) => setForm(f => {
    const steps = [...f.steps];
    ...
    try {
      await createProcedure({
        ...
        steps: form.steps.map(s => ({
          step_number: s.step_number, title: s.title.trim(),
          description: s.description.trim(), note: s.note.trim() || null,
        })),
      });
  ...
  const handleDelete = async (id, title) => {
    ...
    try { await deleteProcedure(id); loadProcedures(); }
    catch { alert("Failed to delete procedure."); }
  };
          ...
          {tabs.map(t => (
            <button key={t.id}
              style={activeTab === t.id
                ? { ...s.tab, ...s.tabActive }
                : t.id === "procedures" ? { ...s.tab, color: "#818cf8" } : s.tab}
        ...
        {activeTab === "overview" && (
          <section>
            <div style={s.statsGrid}>
              <StatCard label="Total Users"  value={usersLoading ? "…" : users.length}          icon="👥" accent="#f59e0b" />
              ...
              <StatCard label="Announcements" value={announcements.length}                       icon="📣" accent="#ef4444" />
            </div>
            <div style={s.twoCol}>
              <div>
              ...
              <div>
                ...
                <div style={s.actionGrid}>
                  {[
                    ...
                    { label:"Departments",   desc:"View all departments", icon:"🏛️" },
                  ].map(a => (
          ...
          <section>
            <div style={s.tableHeader}>
              <h2 style={s.sectionTitle}>
                User Management {!usersLoading && <span style={{ color: "#fbbf24" }}>({users.length})</span>}
              </h2>
              <div style={s.filterRow}>
                {["all","student","faculty","staff","admin","event_coordinator"].map(r => (
                  <button key={r}
            ...
            {usersLoading ? (
              <p style={s.statusText}>Loading users…</p>
            ) : (
              <div style={s.table}>
                <div style={s.tableHeadRow}>
                  <span style={s.thCell}>Name</span>
                  <span style={s.thCell}>Email</span>
                  <span style={s.thCell}>Role</span>
                  <span style={s.thCell}>Created At</span>
                </div>
                {filteredUsers.length === 0 ? (
                  <p style={{ ...s.statusText, padding: "20px" }}>No users found.</p>
                ) : filteredUsers.map(u => (
                  <div key={u.id} style={s.tableRow}>
                    <span style={s.tdName}>{u.name}</span>
                    <span style={s.tdCell}>{u.email}</span>
                    <span style={{ ...s.rolePill, color: ROLE_COLORS[u.role]||"#818cf8", borderColor: `${ROLE_COLORS[u.role]||"#818cf8"}33` }}>
                      {u.role?.replace("_"," ")}
                    </span>
                    <span style={s.tdCell}>{u.created_at ? new Date(u.created_at).toLocaleDateString() : "—"}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <section>
            ...
            <div style={s.searchRow}>
              <input type="text" placeholder="Search departments…" value={deptSearch}
                ...
                style={s.searchInput} />
              <button style={s.searchBtn} onClick={() => fetchDepartments(deptSearch.trim())} disabled={deptLoading}>
              ...
              {procedures.map(proc => (
                <div key={proc.id} style={s.procRow}>
                  <div style={s.procRowLeft}>
                    <span style={{ ...s.catDot, background:
                      proc.category === "academic"     ? "#6366f1" :
                      proc.category === "financial"    ? "#10b981" :
                      proc.category === "registration" ? "#8b5cf6" :
                      proc.category === "it_services"  ? "#0ea5e9" : "#f59e0b"
                    }} />
                    <div>
                  ...
                  <button style={s.deleteBtn} onClick={() => handleDelete(proc.id, proc.title)}>Delete</button>
                </div>
              ))}
            </div>
          </section>
