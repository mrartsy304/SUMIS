        ...
        {activeTab === "announcements" && (
          <section>
            ...
            {announcements.length === 0 ? (
              <p style={{ color: "#475569", fontFamily: "monospace", fontSize: 13 }}>No announcements.</p>
            ) : announcements.map(a => (
              <div key={a.id} style={styles.announcementCard}>
                <div style={styles.annHeader}>
                  <p style={styles.annTitle}>{a.title}</p>
                  <span style={styles.annDate}>{a.created_at ? new Date(a.created_at).toLocaleDateString() : ""}</span>
                </div>
                <p style={styles.annMessage}>{a.body}</p>
              </div>
            ))}
          </section>
        )}
...
const styles = {
  ...
  statsGrid:        { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16, marginBottom: 36 },
