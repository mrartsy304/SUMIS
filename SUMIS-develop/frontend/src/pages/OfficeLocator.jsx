...
// Proper office locations used as fallback when backend has no office_location set
const DEFAULT_OFFICES = {
  faculty: [
    "First Floor, Dept of Computer Science, Room 101",
    "Second Floor, Dept of Software Engineering, Room 201",
    "Third Floor, Dept of Artificial Intelligence, Room 301",
    "Fourth Floor, Dept of Data Science, Room 401",
    "Second Floor, Dept of Electrical Engineering, Room 210",
  ],
  staff: [
    "Ground Floor, Administration Block, Room 005",
    "Ground Floor, Administration Block, Room 010",
    "First Floor, Student Centre, Room 110",
    "First Floor, Administration Block, Room 115",
  ],
  event_coordinator: [
    "First Floor, Student Centre, Room 108",
  ],
};

// Returns a deterministic office string for a given staff member
function resolveOffice(member) {
  if (member.office_location && member.office_location !== "TBA" && member.office_location.trim() !== "") {
    return member.office_location;
  }
  const pool = DEFAULT_OFFICES[member.role] || DEFAULT_OFFICES.staff;
  return pool[member.id % pool.length];
}
              ...
              displayed.map(member => (
                <div key={member.id}
                  ...
                  <div style={styles.cardInfo}>
                    ...
                    <p style={styles.cardDept}>{member.department || "Department not assigned"}</p>
                    <p style={styles.cardLocation}>📍 {resolveOffice(member)}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div style={styles.detailPanel}>
            {selectedMember ? (
              <div style={styles.detailCard}>
                ...
                <DetailRow icon="📍" label="Office" value={resolveOffice(selectedMember)} />
                ...
                {deptDetail ? (
                  ...
                  <div style={styles.deptMock}>
                    ...
                    {selectedMember.office_location && (
                      <p style={styles.deptMockLocation}>📍 {resolveOffice(selectedMember)}</p>
                    )}
                  </div>
                )}
              </div>
...
const styles = {
  ...
  deptMockLocation: { margin: 0, fontSize: 13, color: "#64748b" },
