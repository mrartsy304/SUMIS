...
import { departmentAPI, eventsAPI, notificationsAPI } from "../services/api";
...
export default function StudentPortal() {
  ...
  // Load events and notifications from API
  useEffect(() => {
    eventsAPI.getAll()
        ...
        {activeTab === "overview" && (
          <section>
            <div style={s.statsGrid}>
              ...
              <StatCard label="Notifications"    value={notifications.filter(n => !n.is_read).length} icon="🔔" accent="#10b981" sub={notifications.length + " total"} />
            </div>
            <div style={s.section}>
                        ...
                        {(selectedProc.steps || [])
                          .map((step, idx) => (
                            <div key={idx} style={s.step}>
                              <div style={{ ...s.stepNum, background: catColor }}>{idx + 1}</div>
                              <div style={s.stepContent}>
                                {typeof step === "string" ? (
                                  <p style={s.stepDesc}>{step}</p>
                                ) : (
                                  <>
                                    {step.title && <p style={s.stepTitle}>{step.title}</p>}
                                    {step.description && <p style={s.stepDesc}>{step.description}</p>}
                                    {step.note && <div style={s.stepNote}>💡 {step.note}</div>}
                                  </>
                                )}
                              </div>
                            </div>
